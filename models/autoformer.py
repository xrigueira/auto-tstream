...
class Autoformer(nn.Module):
    ...
    def forward(self, x_enc, x_dec, x_mark_enc, x_mark_dec,
        ...
        zeros = torch.zeros([x_dec.shape[0], self.output_sequence_len, x_dec.shape[2]], device=x_enc.device)
        seasonal_init, trend_init = self.decomp(x_enc)
        
        # decoder input
        trend_init = torch.cat([trend_init[:, -self.decoder_sequence_len:, :], mean], dim=1)
        seasonal_init = torch.cat([seasonal_init[:, -self.decoder_sequence_len:, :], zeros], dim=1)
